...
#Função para  verificar os  services do Mikrotik
def obter_service_mikrotik(conexao_ssh):
    try:
        comando = "/ip service print"
        ...
        print(f"Erro ao obter o service do MikroTik: {e}")
...
def executar_comandos(conexao_ssh):
    while True:
        comando = input("Digite um comando para executar no MikroTik (ou 'sair' para encerrar, 'versao' para obter a versão, 'service' para obter o ip service'upgrade' para enviar arquivo de firmware com opção de reboot): ")
        ...
        elif comando.lower() == 'service':
            obter_service_mikrotik(conexao_ssh)
            input("Pressione Enter para continuar...")
